</body>
<footer>
    <div class="w-1/3" id="footer1">
        <div>
            <h2 class="text-left text-white text-xl ml-20 mt-5">Suivez-nous</h2>
            <br />
            <div id="logos">
                <div class="ml-5"><a href="https://www.facebook.com/"><img class="w-20" src="./assets/images/SeekPng.com_white-circle-png_185630.png" alt="facebook"></a></div>
                <div><a href="https://twitter.com/"><img class="w-20 pt-1" src="./assets/images/SeekPng.com_twitter-png-images_2538676.png" alt="twitter"></a></div>
                <div><a href="https://www.instagram.com/"><img class="w-20" src="./assets/images/SeekPng.com_tidal-icon-png_1383474.png" alt="instagram"></a></div>
            </div>
        </div>
        <div>
            <h2 class="text-center text-white text-xl mt-5">Inscrivez-vous à notre Newsletter</h2>
            <br />
            <a href=""><img class="w-20 ml-20" src="./assets/images/kindpng_4649229.png" alt="mailbox"></a>
        </div>
        <div>
            <h2 class="text-right text-white text-xl mr-20 mt-5">Trouvez nos produits...</h2>
            <br />
            <a href="https://www.google.fr/maps" alt="map"><img class="w-20 ml-10" src="./assets/images/kindpng_1957108.png" alt=""></a>
        </div>
    </div>
    <div class="w-1/3" id="footer2">
        <div>
            <a href="">
                <h2 class="text-left text-white text-xl ml-10 pt-5">La Phytothérapie</h2>
            </a>
            <br />
            <a href="">
                <h2 class="text-left text-white text-xl ml-10 pt-5">Nos Produits</h2>
            </a>
        </div>
        <div>
            <a href="">
                <h2 class="text-center text-white text-xl pt-5">Nous Contacter</h2>
            </a>
        </div>
        <div>
            <a href="">
                <h2 class="text-white text-xl pt-5 -mr-10">Mentions Légales</h2>
            </a>
            <br />
            <a href="">
                <h2 class="text-white text-xl -mr-10">Politique de Confidentialité</h2>
            </a>
            <br />
            <a href="">
                <h2 class="text-white text-xl -mr-10">Conditions générales d'utilisation</h2>
            </a>
        </div>
    </div>
    <!-- <div id="footer">
    <img id="footer" src="./assets/images/footer_phyto.png" alt="footer_phyto">
</div> -->
</footer>

</html>